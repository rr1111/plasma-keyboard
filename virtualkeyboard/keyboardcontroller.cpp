...
void KeyboardController::setLayoutId(const QString &layoutId)
{
    ...
    if (resolvedId.isEmpty()) {
        resolvedId = layoutForLocale(m_inputEngine->locale());
    }
...
QStringList KeyboardController::layoutIdsForLocales(const QStringList &locales) const
{
    QStringList layoutIds;
    for (const auto &locale : locales) {
        const QString layoutId = layoutForLocale(locale);
        if (!layoutId.isEmpty() && !layoutIds.contains(layoutId)) {
...
QString KeyboardController::layoutForLocale(const QString &locale) const
{
    const auto layouts = m_packageResolver->keyboardLayoutIds();
    const QString languageCode = locale.section(QLatin1Char('_'), 0, 0);

    for (const auto &layoutId : layouts) {
        if (m_packageResolver->keyboardLayoutLocales(layoutId).contains(locale)) {
            return layoutId;
        }
    }

    for (const auto &layoutId : layouts) {
        for (const auto &entry : m_packageResolver->keyboardLayoutLocales(layoutId)) {
            if (entry.section(QLatin1Char('_'), 0, 0) == languageCode) {
                return layoutId;
            }
        }
    }

    const QString fallbackId = QStringLiteral("org.kde.plasma.keyboard.en/qwerty");
    if (layouts.contains(fallbackId)) {
        return fallbackId;
    }

    return layouts.isEmpty() ? QString() : layouts.constFirst();
}
